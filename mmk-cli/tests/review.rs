...
#[serial(cwd)]
#[test]
fn review_quality_floor_fires_once_in_200_to_500_band() {
    // Review-effectiveness floor: a 214-LOC diff sits above the
    // 200-LOC absolute floor and below 50% of the per-diff cap
    // (1000), so the ReviewQuality tier fires Info exactly once.
    // A subsequent re-run with the working tree unchanged hits
    // MonotonicSignal and emits zero BUDGET findings.
    use std::fmt::Write as _;
    let dir = TempDir::new().unwrap();
    let now = 1_700_000_000_i64;
    init_repo(dir.path());
    ...
    let mut body = String::with_capacity(214 * 8);
    for i in 0..214 {
        writeln!(body, "line{i}").unwrap();
    }
    write(dir.path(), "seed.rs", &body);

    let count_floor = |stdout: &[u8]| -> usize {
        let v: Value = serde_json::from_slice(stdout).expect("valid JSON");
        v["findings"].as_array().map_or(0, |a| {
            a.iter()
                .filter(|f| {
                    f["layer"] == "budget"
                        && f["message"]
                            .as_str()
                            .unwrap_or("")
                            .contains("review effectiveness degrades")
                })
                .count()
        })
    };
    ...
    assert_eq!(
        count_floor(&stdout1),
        1,
        "ReviewQuality floor must fire exactly once on a 214-LOC diff; got: {}",
        String::from_utf8_lossy(&stdout1)
    );
    let v: Value = serde_json::from_slice(&stdout1).expect("valid JSON");
    let floor_msg = v["findings"]
        .as_array()
        .unwrap()
        .iter()
        .find(|f| f["layer"] == "budget")
        .expect("budget finding present")["message"]
        .as_str()
        .unwrap()
        .to_string();
    assert!(
        floor_msg.contains("review effectiveness degrades")
            && floor_msg.contains("Commit this slice"),
        "floor message must anchor to behavior and name the action; got: {floor_msg}"
    );
    ...
    assert_eq!(
        count_floor(&stdout2),
        0,
        "identical re-run must be suppressed by MonotonicSignal; got: {}",
        String::from_utf8_lossy(&stdout2)
    );
}
